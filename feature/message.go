...
func MessageAction(c *gin.Context) {
	...
	K := GetKey(u.ID, Tuid)
	conf.DB.Model(&Message{}).Count(&MessageId)
	//MessageId++
	if a_t == "1" {
		...
		conf.DB.Model(&Message{}).Create(&m)
	}
...
func MessageChat(c *gin.Context) {
	...
	var count int
	if num.(int) == 0 {
		conf.DB.Model(&Message{}).Where("M_key = ?", K).Find(&MList)
		conf.DB.Model(&Message{}).Where("M_key = ? ", K).Count(&count)
		...
		conf.DB.Model(&Message{}).Where("M_key = ? ", K).Count(&count)
		conf.DB.Model(&Message{}).Where("M_key = ? ", K).Last(&MList)
